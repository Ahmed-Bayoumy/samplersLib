    ...
    def calculate(self, h: float = None):
        ...
        for i in range(self._points.shape[0]):
            for j in range(self._ns):
                if self._nd == 1:
                    K[i] += self.kf_univar((self._points[i]-self.data[j]))
                else:
                    K[i] += self.kf_multivar((self._points[i]-self.data[j]))

            K[i] /= ((self._points.shape[0]-1))
        if h is not None and self._type == KERNEL_TYPE.NONPARAMETRIC:
...
class Laplace(kernel):
    ...
    def kf_univar(self, u):
        h = self.h[0] if (isinstance(self.h, list) or isinstance(self.h, np.ndarray)) and len(self.h) == 1 else self.h
        return (1 / (2 * np.prod(h))) * np.exp(-np.abs(u) / np.prod(h))
...
class cosine(kernel):
    ...
    def kf_multivar(self, u):
        return self.bounded((1 / (2 * np.prod(self.h))**len(u)) * np.cos(np.pi * np.prod(u) / np.prod(self.h)), np.prod(u))
...
class Gaussian_RBF(kernel):
    ...
    def kf_multivar(self, u):
        return np.exp(-np.linalg.norm(u)**2 / (2 * self.h**2))


@dataclass
class Multiquadric_RBF(kernel):
    def __init__(self, data: np.ndarray = None, vlim=None, res: int = 101, weights: Any = None, bw_method=TUNING_METHOD.SCOTT.name, point: np.ndarray = None, n_r: int = 0):
        self.data = copy.deepcopy(data)
        self._ns = data.shape[0]
        self._nd = data.shape[1]
...
@dataclass
class InverseMultiquadric_RBF(kernel):
    def __init__(self, data: np.ndarray = None, vlim=None, res: int = 101, weights: Any = None, bw_method=TUNING_METHOD.SCOTT.name, point: np.ndarray = None, n_r: int = 0):
        self.data = copy.deepcopy(data)
        self._ns = data.shape[0]
        self._nd = data.shape[1]
    ...
    def kf_multivar(self, u):
        return 1 / np.sqrt(np.linalg.norm(u)**2 + self.h**2)


@dataclass
class ThinPlateSpline_RBF(kernel):
    def __init__(self, data: np.ndarray = None, vlim=None, res: int = 101, weights: Any = None, bw_method=TUNING_METHOD.SCOTT.name, point: np.ndarray = None, n_r: int = 0):
        self.data = copy.deepcopy(data)
        self._ns = data.shape[0]
        self._nd = data.shape[1]
