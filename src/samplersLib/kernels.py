...
class Kernel(ABC):
    ...
    def replace_cov(self, cov):
        self._cov = cov
        try:
            det = np.linalg.det(cov)
            self.norm_const = 1.0 / np.sqrt((2 * np.pi) ** 2 * det)
    ...
    def tune_bandwidth_mlcv(self, grid_factor=2.0, max_iter=50, random_restarts=3):
        ...
        for _ in range(random_restarts):
            rand_start = np.array([self.rng.uniform(low=b[0], high=b[1]) for b in bounds])
    ...
    def bounded(self, f, u):
        """
        Return f if all components of u are within their bandwidths,
        otherwise return 0.
        """
        # If u is a scalar (e.g., from the univariate kernel)
        # if np.isscalar(u) or isinstance(u, (int, float)):
        #     return f if abs(u) < self.h else 0

        # For a vector: check every component against its bandwidth
        within_bounds = np.abs(u) < np.prod(self.h)  # element‑wise comparison
        return f if np.all(within_bounds) else 0
    ...
    def _calc_covariance(self):  # noqa: C901
        """
        ...
        try:
            ...
            if self._data_inv_cov is None:
                ...
                # Compute raw data covariance (unscaled)
                self._data_cov = np.atleast_2d(np.cov(self.data.T, rowvar=1, bias=True, aweights=self.weights))

                # Regularise the covariance if it is singular
                try:
                    self._data_inv_cov = np.linalg.inv(self._data_cov)
                except np.linalg.LinAlgError:
                    # Add a small jitter to the diagonal and retry
                    jitter = 1e-6 * np.eye(self._nd)
                    cov_reg = self._data_cov + jitter
                    try:
                        self._data_inv_cov = np.linalg.inv(cov_reg)
                    except np.linalg.LinAlgError:
                        # As a last resort use the pseudo‑inverse
                        self._data_inv_cov = np.linalg.pinv(cov_reg)

            # Scale the covariance by factor**2 to obtain the kernel covariance
            ...
            # Invert the scaled covariance (use the same regularisation logic)
            if self._inv_cov is None and self._cov is not None:
                try:
                    self._inv_cov = np.linalg.inv(self._cov)
                    det = np.linalg.det(self._cov)
                    if det <= 0:
                        raise ValueError("Covariance determinant non‑positive")
                    self.norm_const = 1.0 / np.sqrt((2 * np.pi) ** self._nd * det)
                except (np.linalg.LinAlgError, ZeroDivisionError, ValueError):
                    # --- Step 1: jitter the covariance ---------------------------------
                    jitter = 1e-6 * np.eye(self._nd)
                    cov_reg = self._cov + jitter
                    try:
                        ...
                        # --- Step 2: pseudo‑inverse & pseudo‑determinant -----------------
                        self._inv_cov = np.linalg.pinv(self._cov)
                        # Compute the product of non‑zero singular values
                        s_vals = np.linalg.svd(self._cov, compute_uv=False)
                        det = np.prod(s_vals[s_vals > 1e-12])  # ignore tiny singulars
                        self.norm_const = 1.0 / np.sqrt((2 * np.pi) ** self._nd * det)

        except Exception:
            # Reset all cached matrices on failure
            self._data_cov = None
    ...
    def _estimate_noise_var(self, init_frac: float = 1e-6, max_iter: int = 10, tol: float = 1e-4) -> float:
        """
        Roughly estimate observation noise variance from data.
        Uses a simple EM‑style update that converges quickly for small noise.
        ...
        init_frac : float
            Initial guess as a fraction of the sample variance.
        max_iter : int
        ...
        """
        var_y = np.var(self.data, ddof=1)
        sigma2 = init_frac * var_y

        for _ in range(max_iter):
            # Expected signal variance (assuming unit kernel)
            sig_sqr = var_y - sigma2
            if sig_sqr <= 0:
                break
            new_sigma2 = max(1e-12, var_y - sig_sqr)  # keep positive
            if abs(new_sigma2 - sigma2) / sigma2 < tol:
                break
            sigma2 = new_sigma2
    ...
    def _sample_noise(self):  # noqa: C901
        ...
        if kernel_name == "gaussian" or kernel_name == "gaussianrbf" or kernel_name == "mixturegaussian":
            # Box-Muller for Normal Distribution
            return [
                h[i] * np.sqrt(-2 * np.log(self.rng.random())) * np.cos(2 * np.pi * self.rng.random()) for i in range(self._nd)
            ]

        elif kernel_name == "cauchy":
            # Inverse Transform Sampling: h * tan(pi * (U - 0.5))
            return [h[i] * np.tan(np.pi * (self.rng.random() - 0.5)) for i in range(self._nd)]

        elif kernel_name == "laplace":
            # Difference of two exponentials
            return [h[i] * (np.log(self.rng.random()) - np.log(self.rng.random())) for i in range(self._nd)]

        elif kernel_name == "logistic":
            # h * log(U / (1-U))
            return [h[i] * np.log(u / (1 - u)) if (u := self.rng.random()) else 0 for i in range(self._nd)]
        # NEW: SparseGPFitc – Gaussian noise with variance self.noise_var
        elif kernel_name == "sparsegpfitc":
            return [h[i] * self.rng.randn() for i in range(self._nd)]

        # NEW: ArdRbfSharedLengthScale – Gaussian noise with std = length_scale
        elif kernel_name == "ardrbfshared":
            return [h[i] * self.rng.randn() for i in range(self._nd)]
        ...
        # Using Rejection Sampling for complex shapes
            ...
            for i in range(self._nd):
                passed = False
                while True:
                    u, v = self.rng.uniform(-1, 1), self.rng.random()
        ...
        # 3. RBF-Specific Sampling (Approximated for Generative Use)
            ...
            return [h[i] * self.rng.normal(0, 2) for i in range(self._nd)]
...
class Gaussian(Kernel):
    ...
    def kf_multivar(self, z: np.ndarray):
        """
        Evaluate a multivariate Gaussian kernel at point `z`.

        Handles singular covariance matrices by:
          1. Trying the exact inverse/determinant.
          2. If that fails, adding a tiny jitter to make it invertible.
          3. As a last resort, using the pseudo‑inverse and the product of
            non‑zero singular values as an approximate determinant.
        """
        if self._inv_cov is not None and not np.isnan(self.norm_const) and not np.isinf(self.norm_const):
            exponent = -0.5 * z.T @ self._inv_cov @ z
            val = self.norm_const * np.exp(exponent)
...
class Cauchy(Kernel):
    ...
    * In multivariate mode a covariance matrix may be supplied; if it is
    ...
    def kf_multivar(self, z: np.ndarray) -> float:
        """
        Evaluate a multivariate Cauchy kernel at point `z`.

        Handles singular covariance matrices by:
          1. Checking the determinant first.
          2. If det <= 0 or inversion fails, fall back to jittered
            or pseudo‑inverse covariance.
        """
        d = self._nd  # dimensionality of the data

        if self._inv_cov is not None:
            quad_form = z.T @ self._inv_cov @ z

            # Compute in log‑space to avoid underflow
            log_prefactor = np.log(gamma((d + 1.0) / 2.0)) - (d / 2.0) * np.log(np.pi) - np.log(gamma(0.5))
            log_term = -(d + 1.0) / 2.0 * np.log1p(quad_form)

            return np.exp(log_prefactor + log_term)

        # -------------------------------------------------------------------------
        # No covariance matrix – fall back to the bandwidth‑only formulation
        if self.h is None:
...
class Cosine(Kernel):
    ...
    * In the multivariate case, the kernel remains separable only through the
    ...
    def kf_multivar(self, u):
        """
        Multivariate cosine kernel.
        For each dimension j:
            K_{h_j}(u_j) = (1/(2*h_j)) * cos(pi*u_j/h_j)  if |u_j| <= h_j else 0
        The multivariate kernel is the product over all dimensions.
        """
        ...
        # Compute the product of cosines and the normalisation constant
        prod_u = np.prod(u)
        prod_h = np.prod(self.h)
        val = (1 / (2 * prod_h)) ** len(u) * np.cos(np.pi * prod_u / prod_h)
        return self.bounded(val, u=u)  # keep the original vector for boundedness check
...
class Linear(Kernel):
    ...
    def kf_multivar(self, u):
        ...
        float
            Kernel weight for ``u``.
        """
        if (isinstance(self.h, list) or isinstance(self.h, np.ndarray)) and len(self.h) == 1:
            h = self.h[0]
            return max(0.0, 1 - np.linalg.norm(u) / h)
        else:
            h = self.h
            return np.prod([max(0.0, 1 - abs(ui) / h[i]) for i, ui in enumerate(u)])
...
class SparseGpFitc(Kernel):
    """
    FITC sparse Gaussian‑process kernel.
    Parameters
    ----------
    data : np.ndarray, optional
        Training data – used only to initialise inducing points if `h` is None.
    vlim : Any, optional
        Value limits – kept for API compatibility.
    weights : Any, optional
        Sample weights – ignored in this kernel.
    bw_method : str, default="SCOTT"
        Bandwidth method – ignored; placeholder for API consistency.
    n_r : int, default=0
        Number of reference points – unused.
    h : List[float], optional
        If provided, the first element is interpreted as the number of inducing
        points (`m`).  Otherwise a default of ``10`` is used.
    calculate_bw : bool, default=True
    ...
    def __init__(
        ...
        # Randomly initialise inducing points in the input space.
        self.inducing = self.rng.normal(size=(self.m, self._nd))

        # Pre‑compute K_uu and its inverse (Cholesky for stability)
        self.K_uu = self._rbf(self.inducing, self.inducing)
        jitter = 1e-6 * np.eye(self.m)
        self.L_uu = np.linalg.cholesky(self.K_uu + jitter)
        self.inv_K_uu = np.linalg.solve(self.L_uu.T, np.linalg.solve(self.L_uu, np.eye(self.m)))

    # ------------------------------------------------------------------
    def _rbf(self, X: np.ndarray, Y: np.ndarray) -> np.ndarray:
        """Batch RBF kernel (variance=1.0)."""
        dists = cdist(X / self.h, Y / self.h, metric="sqeuclidean")
        return np.exp(-0.5 * dists)

    # ------------------------------------------------------------------
    def kf_univar(self, u):
        """Univariate FITC kernel – not used but kept for API consistency."""
        return 1.0

    # ------------------------------------------------------------------
    def kf_multivar(self, z: np.ndarray):
        """
        Evaluate the FITC sparse GP kernel at point(s) `z`.
        Returns a scalar (or array if `z` is batched).
        """
        # Full covariance term
        k_ff = self._rbf(np.atleast_2d(z), np.atleast_2d(z))[0, 0]

        # Inducing‑to‑data covariances
        k_fu = self._rbf(np.atleast_2d(z), self.inducing).flatten()
        k_vu = self._rbf(self.inducing, np.atleast_2d(z)).T.flatten()

        # Correction term: k_fu @ K_uu⁻¹ @ k_vu
        corr = k_fu @ (self.inv_K_uu @ k_vu)
...
class ArdRbfShared(Kernel):
    """
    Radial Basis Function kernel with a single global length‑scale.
    Parameters
    ----------
    data : np.ndarray, optional
        Training data used to estimate the bandwidth (ignored here).
    vlim : Any, optional
        Value limits – kept for API compatibility.
    weights : Any, optional
        Sample weights – ignored in this kernel.
    ...
    n_r : int, default=0
        Number of reference points – unused.
    h : List[float], optional
        Explicit bandwidth(s).  If provided, the first element is used as
        the global length‑scale.  Otherwise a default of ``1.0`` is used.
    calculate_bw : bool, default=True
        Ignored; kept for API compatibility.
    """
    ...
    def __init__(
        ...
        # Use the first bandwidth value as a global length‑scale.
        if self.h is not None:
            self.length_scale = float(self.h[0])
        else:
            self.length_scale = 1.0

        # Cache log‑normalisation constant for speed
        self._log_norm_const = -0.5 * self._nd * np.log(2 * np.pi) - self._nd * np.log(self.length_scale)

    # ------------------------------------------------------------------
    def kf_univar(self, u):
        """Univariate RBF (same as Gaussian with σ=length_scale)."""
        h = self.length_scale
        return (1 / (np.sqrt(2 * np.pi) * h)) * np.exp(-0.5 * (u / h) ** 2)
    ...
    def kf_multivar(self, z: np.ndarray):
        """
        Evaluate the multivariate RBF kernel at point(s) `z`.
        Uses the cached log‑normalisation constant and a shared length‑scale.
        """
        ...
        return np.exp(self._log_norm_const - 0.5 * np.sum(scaled_z**2, axis=-1))


# src/samplersLib/kernels.py


class MixtureGaussian(Kernel):
    """Mixture of multivariate Gaussian kernels.

    Parameters
    ----------
    data : np.ndarray, optional
        Data used to estimate the kernel (not required for evaluation).
    vlim : tuple, optional
        Value limits for each dimension.
    weights : array‑like, optional
        Observation weights.
    bw_method : str, default="SCOTT"
        Bandwidth selection method.
    n_r : int, default=0
        Number of random points to use in bandwidth estimation.
    h : list or np.ndarray, optional
        Diagonal bandwidths for each component (used if `cov` is None).
    mu : array‑like, shape (n_components, ndim)
        Means of the Gaussian components.
    cov : array‑like or list of array‑like, optional
        Covariance matrix(es).  If a single matrix is supplied it is shared
        across all components; otherwise a list/tuple of matrices must be
        provided with length equal to ``n_components``.
    mixture_weights : array‑like, shape (n_components,)
        Mixing proportions.  They are normalised internally.
    calculate_bw : bool, default=True
        Whether to compute bandwidths automatically.

    Notes
    -----
    The kernel value at a point `z` is

    .. math::
       K(z) = \\sum_{k=1}^{K} w_k \\, N(z | \\mu_k, \\Sigma_k)

    where :math:`w_k` are the mixture weights.
    """
    ...
    def __init__(  # noqa: C901
        ...
        if mu is None:
            mu = np.zeros((len(data), len(self.h)))
        self.mu = np.asarray(mu)
        self.n_components, self._nd = self.mu.shape

        # Normalise mixture weights
        if mixture_weights is None:
            self.mixture_weights = np.ones(self.n_components) / self.n_components
        else:
            mw = np.asarray(mixture_weights, dtype=float)
            if mw.size != self.n_components:
                raise ValueError("Length of `mixture_weights` must match number of components.")
            self.mixture_weights = mw / np.sum(mw)

        # Handle covariance(s)
        if self._cov is None:
            if h is None:
                raise ValueError("Either `cov` or diagonal bandwidths `h` must be provided.")
            # Use diagonal bandwidths as identity matrices scaled by h
            self.cov_matrices = [np.diag(h) for _ in range(self.n_components)]
        else:
            cov_arr = np.asarray(self._cov)
            if cov_arr.ndim == 2:  # single shared covariance
                self.cov_matrices = [cov_arr] * self.n_components
            elif cov_arr.ndim == 3 and cov_arr.shape[0] == self.n_components:
                self.cov_matrices = [cov_arr[i] for i in range(self.n_components)]
            else:
                raise ValueError("`cov` must be (ndim, ndim) or (n_components, ndim, ndim).")

        # Pre‑compute inverses and normalisation constants per component
        self._inv_covs = []
        self._norm_consts = []
        for C in self.cov_matrices:
            try:
                invC = np.linalg.inv(C)
                detC = np.linalg.det(C)
                if detC <= 0 or np.isnan(detC):
                    raise np.linalg.LinAlgError
            except Exception:
                # fallback: add jitter and recompute
                eps = 1e-8 * np.eye(self._nd)
                C += eps
                invC = np.linalg.inv(C)
                detC = np.linalg.det(C)

            self._inv_covs.append(invC)
            self._norm_consts.append((2 * np.pi) ** (-self._nd / 2.0) * detC**-0.5)

    def kf_univar(self, u: np.ndarray):
        """Evaluate the mixture kernel at 1‑D point(s) `u`.

        Parameters
        ----------
        u : array‑like, shape (n_points,) or scalar
        ...
        vals = np.zeros_like(u, dtype=float)
        for k in range(self.n_components):
            # For univariate case the covariance reduces to a scalar variance
            var_k = self.cov_matrices[k].flat[0] if self.cov_matrices[k].ndim == 2 else self.cov_matrices[k]
            std_k = np.sqrt(var_k)
            diff = u - self.mu[k, 0]
            exponent = -0.5 * (diff / std_k) ** 2
            vals += self.mixture_weights[k] * (1 / (np.sqrt(2 * np.pi) * std_k)) * np.exp(exponent)
        return vals

    def kf_multivar(self, z: np.ndarray):
        """Evaluate the mixture kernel at point(s) `z`.

        Parameters
        ----------
        z : array‑like, shape (n_points, ndim)
        ...
        vals = np.zeros(z.shape[0])
        for k in range(self.n_components):
            diff = z - self.mu[k]
            exponent = -0.5 * np.sum(diff @ self._inv_covs[k] * diff, axis=1)
            vals += self.mixture_weights[k] * self._norm_consts[k] * np.exp(exponent)
        return vals[0]
